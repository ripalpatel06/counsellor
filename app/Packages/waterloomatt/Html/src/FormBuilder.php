...
class FormBuilder extends IlluminateFormBuilder
{
    ...
    {
        $options = $this->appendClassToOptions('form-group row has-feedback', $options);
        ...
        {
            // If the form element with the given name has any errors, apply the 'has-error' class to the group.
            $options = $this->appendClassToOptions('has-error', $options);
        }

        // If a label is given, we set it up here. Otherwise, we will just set it to an empty string.
        $label = $label ? $this->label($name, $label) : '';

        /**
         * Render an open div before the label to control the input size.
         * <div class="col-xs-4">
         *     <label>...
         *     <input>...
         *
         * closeGroup() will close the open div
         */
        $size = array_pull($options, 'size', 'col-xs-12');
        $label = $this->wrapInput($size, $label);
    ...
    private function getFormattedErrors($name)
    {
        ...
        return $errors->first($this->transformKey($name), '<span class="has-error"><p class="help-block">:message</p></span>');
    }
    ...
    private function wrapInput($class, $input)
    {
        $this->_wrapInput = true;

        return '<div class="'.$class.'">'.$input;
    }
}